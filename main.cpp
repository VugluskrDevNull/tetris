...
bool DoesPieceFit(int nTetromino, int nRotation, int nPosX, int nPosY)
{
    for (int px = 0; px < 4; px++)       // All Field cells >0 are occupied
        for (int py = 0; py < 4; py++)
        {
            ...
            // Check that test is in bounds. Note out of bounds does not necessarily mean a fail, as the long vertical piece
            // can have cells that lie outside the boundary, so we'll just ignore them
